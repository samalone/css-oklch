...
- `src/formulaPanel.ts` — Webview-based adaptive color formula panel. Helps design relative color formulas that maintain contrast guarantees across a range of base color values where one OKLCH component varies. Sweeps the variable component, computes min/max APCA/WCAG contrast via sampling + golden section refinement, and displays a contrast chart with threshold lines and min/max markers.
